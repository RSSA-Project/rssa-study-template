...
const MovieCard = ({
  ...
  const ratingMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      queryClient.setQueryData<RatedItem[]>(
        ["movieRatings"],
        (oldRatings: RatedItem[] | undefined) => {
          const existingRatings = oldRatings || [];
          const index = existingRatings.findIndex(
            (rat) => rat.item_id === result.item_id,
          );
          if (index > -1) {
            return existingRatings.map((rat, i) =>
              i === index ? newRatingItem : rat,
            );
          } else {
    ...
    >
      <div className="relative lg:h-54 xxl:h-81 flex items-center justify-center bg-black">
        <img
          className={clsx(
            "h-full w-full",
            isHovered ? "object-contain" : "object-contain",
          )}
        ...
        >
          <div className="absolute bottom-0 w-full pt-4 px-1 flex flex-col items center text-center">
            <p className="text-white text-md font-medium mb-0 leading-tight">
              {movie.title}
            </p>
