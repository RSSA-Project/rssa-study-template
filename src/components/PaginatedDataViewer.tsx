...
const PaginatedResourceViewer = <T extends { id: string }>({
  ...
  const fetchResources = async (page: number) => {
    ...
    return studyApi.get<PaginatedDataList<T>>(
      `${apiResourceTag}/?offset=${offset}&limit=${limit}`,
    );
  };
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="">
        {children(resourceList, selectedItem, handleItemClick)}
      </div>
      <div
        className={clsx(
          "flex-shrink-0 p-4",
      ...
      >
        ...
        <button
          onClick={() =>
            setPage((old) => (page + 1 < totalPages ? old + 1 : old))
          }
