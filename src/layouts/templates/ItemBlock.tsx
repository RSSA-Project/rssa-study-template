...
const ItemBlock: React.FC<ItemBlockProps> = ({
  ...
  const responseUpsertMutation = useMutation({
    ...
    onSuccess: (result) => {
      queryClient.setQueryData<SurveyItemResponse[]>(
        ["currentPageResponses", pageId],
        (oldResponses) => {
          ...
          const index = existingResponses.findIndex(
            (res) => res.survey_item_id === result.item_id,
          );

          if (index > -1) {
            return existingResponses.map((res, i) =>
              i === index ? newResponse : res,
            );
          } else {
    ...
    onError: () => {
      console.error(
        `Failed to save response for item ${item.id}. Rolling back selection.`,
      );
  ...
  return (
    ...
    >
      <Label className="text-left p-3 text-base font-normal">
        {parseHTML(item.display_name)}
      </Label>
