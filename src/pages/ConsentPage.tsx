...
const ConsentPage: React.FC<ConsentPageProps> = ({
  ...
  const effectiveKey =
    participantTypeKey || (ctxKey !== "unknown" ? ctxKey : "unknown");
  ...
  const consentButtonDisabled =
    !agreed || consentMutation.isPending || isStepComplete;
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="bg-white shadow-md rounded-lg p-6 text-left">
        ...
        {title && (
          <p className="informedConsent-title font-bold mt-4">{title}</p>
        )}
        ...
        <p className="font-bold mb-2">
          By participating in the study, you indicate that you have read the
          information written above, been allowed to ask any questions, and you
          are voluntarily choosing to take part in this research.
        </p>
      </div>
        ...
        >
          {agreed && (
            ...
            >
              <path
                fillRule="evenodd"
                d="M19.916 4.626a.75.75 0 0 1 .207 1.05l-9 13.5a.75.75 0 0 1-1.154.114l-6-6a.75.75 0 0 1 1.06-1.06l5.353 5.353 8.493-12.74a.75.75 0 0 1 1.04-.207Z"
                clipRule="evenodd"
              />
            </svg>
          )}
      ...
      {resumeCode && (
        <div className="mt-4">
          ...
          <div>
            <p>
              Please copy the code above. In case the study session fails, you
              may be able to resume the study by using the code.
            </p>
          </div>
        </div>
      )}
