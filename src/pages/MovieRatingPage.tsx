...
const RatingProgress: React.FC<{ completed: number; total: number }> = ({
  completed,
  total,
}) => {
    ...
    <div className="w-full mt-3">
      ...
      <div className="flex justify-between items-center">
        {isComplete && (
          <span>
            <i>
              You are done but feel free to rate more for more accurate
              recommendations.
            </i>
          </span>
        )}
        ...
        <span className="text-xl font-bold text-gray-700">
          {completed} / {total}{" "}
          {isComplete && (
            <CheckCircleIcon className="inline-block w-5 h-5 ml-1 align-text-middle" />
          )}
        </span>
      </div>
      ...
      <div className="justify-between p-3 bg-gray-300 rounded-3">
        ...
        >
          {(movies, _, handleItemClick) => (
            <div
              className={clsx(
                "grid sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-6 gap-3",
              )}
            >
              {movies.length > 0 ? (
                movies.map((movie) => {
