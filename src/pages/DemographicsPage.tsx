...
const DemographicsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const isGenderSectionValid =
      gender !== "Prefer to self-describe" || genderText !== "";
    const isRaceSectionValid =
      !race.has("Not listed (Please specify)") || raceText !== "";
    const isFormValid =
      areBaseFieldsFilled && isGenderSectionValid && isRaceSectionValid;
  ...
  const demographicsMutation = useMutation({
    ...
    mutationFn: async () =>
      studyApi.post<Demographic, Demographic>("participants/demographics", {
        age_range: age!,
        gender: gender!,
