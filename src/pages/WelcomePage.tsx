...
import { useStudyConfig } from "rssa-api";
...
const WelcomePage: React.FC<WelcomePageProps> = ({
  ...
  const resumeMutation = useMutation({
    mutationFn: (resumeCode: string) => {
      return studyApi.post<ResumePayload, ResumeResponse>(
        `studies/${studyId}/resume`,
        {
    ...
    onSuccess: (data: ResumeResponse) => {
      setJwt(data.token);
      const stepFromConfig = config?.steps.find(
        (step) => step.step_id === data.current_step_id,
      );
      if (stepFromConfig) navigate(stepFromConfig.path);
      else
        throw new Error("Something went wrong. Could not resolve resume path.");
    },
    onError: (error) => {
